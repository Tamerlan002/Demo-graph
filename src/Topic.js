...
function Topic() {
  ...
  const [topicData, setTopicData] = useState(null); // State to store fetched lifecycle stage data
  const [subtopicArr, setSubtopicData] = useState(null); // State to store fetched topic data
  ...
  useEffect(() => {
    const fetchStageData = async () => {
      try {
        const response = await fetch(`http://tamerlan029100-001-site1.jtempurl.com/topics/id?id=${id}`);
  ...
  const handleLearnMoreClick = async () => {
    ...
    try {
      const response = await fetch(`http://tamerlan029100-001-site1.jtempurl.com/subtopics/byTopicId?topicId=${id}`);
      const data = await response.json();
      setSubtopicData(data); // Set the fetched topic data into state
  ...
  useEffect(() => {
    if (!subtopicArr || subtopicArr.length === 0 || !graphCreated) return;

    const graph = new Graph();

    // Add nodes to the graph based on fetched data
    subtopicArr.forEach((data, index) => {
      const nodeId = `${index + 1}`;
      graph.addNode(nodeId, {
        label: data.subtopicName,
        id: data.id,
        x: Math.random(),
        y: Math.random(),
        size: 15,
    ...
    for (let i = 0; i < subtopicArr.length - 1; i++) {
    ...
    renderer.on("clickNode", (event) => {
          ...
          const newTab = window.open(`/subtopic/${nodeLinkName}`, "_blank");
          newTab.focus();
        });
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {topicData.topicName} {/* Display the linkName or any other dynamic value */}
        </h1>
        <p
          style={{
            fontSize: "18px",
        ...
        >
          {topicData.definition} {/* Display actual data */}
        </p>
      ...
      {graphCreated && subtopicArr.length > 0 && (
        <div
          style={{
            width: "100%",
            maxWidth: "1000px",
